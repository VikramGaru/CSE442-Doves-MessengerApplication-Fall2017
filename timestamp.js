...
wss.broadcast = function broadcast(s,ws) {
    ...
    wss.clients.forEach(function each(client) {
         if (typeof client.user != "undefined") {
          if(s == 1){
            console.log("[" + Date() + "]: " + "sending " + ws.msg + " to " + client.user);
            client.send("[" + Date() + "]: " + ws.name + ": " + "'"+ ws.msg + "'");
          }
          if(s == 0){
            client.send("[" + Date() + "]: " + ws + " exit chatroom"); 
          }
         }
    });
};
...
wss.on('connection', function(ws, req) {
  ...
  ws.on('message', function(jsonStr,flags) {
			  ...
			  if(firstMessage)
			  {
			    ...
			    console.log("[" + Date() + "]: " + "secret received = " + jsonStr);
			    console.log("[" + Date() + "]: " + "secret stored = " + secret);
			    //When it is not the same, means someone is hacking so the ws closed immediately.
			    if(secret != jsonStr)
			    {
			      secret = null;
			      console.log("[" + Date() + "]: " + "Secret failed, disconnecting");
			      ws.close();
			      return;
			    }
			    secret = null;
    ...
    ws.on('close', function(close) {
        try{
          ...
          console.log("[" + Date() + "]: " + "close, try to refresh.");
        }
    });
