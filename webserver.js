...
wss.on('connection', function(ws, req) {
	...
	ws.on('message', function(jsonStr,flags) {
		...
		if (typeof this.user.msg != "undefined") {
			/*
			check to see if the name(sender) is the same as peername(target)
			if they are the same then its a broadcast message, otherwise
			its a whisper, so find the peer and send it only to them
			*/
			if(this.user.name != this.user.peername) {
				//find the peer client to send a whisper
				wss.clients.forEach(function each(client) {
					if(client.name == this.user.peername) {
						console.log("sending " + ws.msg + " to " + client.user);
						client.send(ws.name + ": " + "'"+ ws.msg + "'");
					}
				}
	...
	return new Promise(function(resolve,reject){
		MongoClient.connect(url, function(err, db) {
			...
			db.collection("Users").find(query).toArray(function(err, result) {
				if (err) {
					reject(err);// if empty array then wrong details entered so show enter correct username/password
				}
				db.close();
				resolve(result);
				});
...
function login(ulog,pass){
	return new Promise(function(resolve,reject){
		...
		promise.then(function(arr){
			if(arr.length!=0){
				if((arr[0].password).localeCompare(pass)==0){
					res=true;console.log("Successfully logged in");
					}
					else{
